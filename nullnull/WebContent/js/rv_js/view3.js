...
$(function() {
	...
	$('ul+.CommentWriter .btn_register').click(function(){
		...
		if(!content){//내용 없이 등록 클릭한경우 
			alert("댓글을 입력하세요 ");
			return;			
		}
		$.ajax({
			url : 'RvCommentAdd.rv',	//원문 등록
			data : {
				id : $("#loginid").val(),
				content : content ,
	...
	$(".comment_list").on('click','.comment_tool_button',function(){
		...
		if($('.comment_list+.CommentWriter').css('display')=='block'){
			...
			if(selector.css('display')=='block'){	//현재 더보기가 열린경우
				//$(".LayerMore") 중에서 selecor가 아닌 객체들의 display속성을 none으로 설정합니다.
...
//더보기 클릭 후 -> 수정 클릭한 경우에 수정 폼으 보여줍니다.
function updateForm(num){ //num : 수정할 댓글 글번호
	...
	$(selector).hide();	//selector 영역 숨겨요 - 수정에서 취소를 선택하면 보여줄 예정
