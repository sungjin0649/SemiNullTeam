	        ...
	        try {
	    	 ...
	    	 //HTTP응답속도보다 자바의 컴파일 속도가 더 빠르기 때문에 임의적으로 1초를 대기한다.
	    		try {Thread.sleep(1000);} catch (InterruptedException e) {}
	    		 ...
	    		 for(int i =0; i< 3; i++) {
	    			 c= new Crawl();
	    			c.setRank(driver.findElements(By.className("rank")).get(i).getText()); //넘버
	    			c.setName(driver.findElements(By.className("title")).get(i).getText()); //제목
	    			c.setTicket(driver.findElements(By.className("percent")).get((i*2)).getText());//예매율
	    			c.setGood(driver.findElements(By.className("percent")).get((i*2)+1).getText());//평점
	    			c.setSrc(driver.findElement(By.xpath("//*[@id=\"contents\"]/div[1]/div[3]/ol[1]/li["+(i+1)+"]/div[1]/a/span/img")).getAttribute("src"));
	    			movielist.add(c);
	    		 }
	    		 for(int i =0; i< 4; i++) {
	    			 c= new Crawl();
	     			c.setRank(driver.findElements(By.className("rank")).get(i+3).getText()); //넘버
	     			c.setName(driver.findElements(By.className("title")).get(i+3).getText()); //제목
	     			c.setTicket(driver.findElements(By.className("percent")).get((i+3)*2).getText());//예매율
	     			c.setGood(driver.findElements(By.className("percent")).get((i+3)*2+1).getText());//평점
