...
function updateForm(num) { //num : 수정할 댓글 글번호
	...
	//선택한 내용을 구합니다.
	var fcm_content=$("#"+num).find('.text_comment').text();
	...
	selector.find('textarea').val(fcm_content);
	...
	selector.find('.comment_inbox_count').text(fcm_content.length+"/200");
...
//	 })//form
	...
	$('ul+.CommentWriter .btn_register').click(function(){
		var fcm_content=$('.comment_inbox_text').val();
		if(!fcm_content){//내용없이 등록 클릭한 경우
			alert("댓글을 입력하세요");
			return;
		}
		...
		$.ajax({
			...
			data : {
				...
				fcm_comment_board_num : $("#comment_board_num").val(),
				fcm_comment_re_lev : 0, //원문일 경우 comment_re_seq는 0,
				                    //comment_re_ref는 댓글의 원문 글번호
				fcm_comment_re_seq: 0
			},
			type : 'post',
			success : function(rdata) {
				if (rdata == 1) {
	...
	$('.CommentBox').on('click','.update',function(){
		...
		var fcm_content = $(this).parent().parent().find('textarea').val();
		if(!fcm_content){
			alert('수정할 내용을 입력하세요');
			return
		}
		$.ajax({
			...
			data: {num:$(this).attr('data-id'), fcm_content:fcm_content},
			success:function(rdata){
				if(rdata==1) {
					getList(option);
				}//if
	...
	$('.CommentBox').on('click','.reply', function(){
		...
		var fcm_content=$(this).parent().parent().find('.comment_inbox_text').val();
		if(!fcm_content){
			alert('답변 내용을 입력하세요');
			return
		}
