function getList(state){
		...
		$.ajax({
			type:"post",
			url:"Bbs_bkCommentList.bk",
			data : {"BCM_COMMENT_BOARD_NUM" : $("#BCM_COMMENT_BOARD_NUM").val() , 
				    state:state},
			dataType:"json",
			 ...
			 if(rdata.boardlist.length>0){
				   output += '<li class="comment_tab_item ' +  red1 + '" >'
					...
					$(rdata.boardlist).each(function(){
						var lev = this.BCM_COMMENT_RE_LEV;
						var comment_reply='';
						//레벨에 따라 왼쪽 여백줍니다.
						if(lev==1){
						...
						output += '<li id="' + this.BCM_NO + '" class="CommentItem' + comment_reply + '">'
							   + '   <div class="comment_area">'
							   + '    <img  src="' + src +'" alt="프로필 사진" width="36" height="36">'
							   + '    <div class="comment_box">'
							   + '      <div class="comment_nick_box">'
							   + '            <div class="comment_nick_info">'
							   + '               <div class="comment_nickname">' + this.BCM_ID  + '</div>'
							   + '            </div>' //comment_nick_info                  
							   + '       </div>'  // comment_nick_box
							   + '     </div>'   //comment_box
							   + '    <div class="comment_text_box">'
							   + '       <p class="comment_text_view">'
							   + '         <span class="text_comment">' + this.BCM_CONTENT + '</span>'
							   + '       </p>'
							   + '    </div>' //comment_text_box
							   + '    <div class="comment_info_box">'
							   + '      <span class="comment_info_date">' + this.BCM_REG_DATE + '</span>';
						if(lev<2){ //답글쓰기는 답글의 답글까지만 사용하도록 합니다.
						   	  output += '  <a href="javascript:replyform(' + this.BCM_NO +',' 
						   	         + lev + ',' + this.BCM_COMMENT_RE_SEQ +',' 
						   	         + this.BCM_COMMENT_RE_REF +')"  class="comment_info_button">답글쓰기</a>'
						      }
						output += '   </div>' //comment_info_box;
							   
						//글쓴이가 로그인한 경우 나타나는 더보기입니다.
                        //수정과 삭제 기능이 있습니다.							
					    if($("#loginid").val()==this.BCM_ID){  
						 output +=  '<div class="comment_tool">'
							   + '    <div title="더보기" class="comment_tool_button">'
							   + '       <div>&#46;&#46;&#46;</div>' 
							   + '    </div>'
							   + '    <div id="commentItem' +  this.BCM_NO + '"  class="LayerMore">' //스타일에서 display:none; 설정함
							   + '     <ul class="layer_list">'							   
							   + '      <li class="layer_item">'
							   + '       <a href="javascript:updateForm(' + this.BCM_NO + ')"'
							   + '          class="layer_button">수정</a>&nbsp;&nbsp;'
							   + '       <a href="javascript:del(' + this.BCM_NO + ')"'
...
function del(BCM_NO){ //num : 댓글 번호
	if(!confirm('정말 삭제하시겠습니까')){
		return;
	}
	
	$.ajax({
		url : 'Bbs_bkCommentDelete.bk',
		data:{BCM_NO:BCM_NO},
		success:function(rdata){
			getList(option);
			/*if(rdata==1){ DAO에서 int로 햇을때
				getList(option);
			}*/
		}
	})
...
$(document).ready(function() {
	...
	$('ul+.CommentWriter .btn_register').click(function() {
		...
		$.ajax({
			url : 'Bbs_bkCommentAdd.bk', //원문 등록
			data : {
				BCM_ID : $("#loginid").val(),
				BCM_CONTENT : content,
				BCM_COMMENT_BOARD_NUM : $("#BCM_COMMENT_BOARD_NUM").val(),
				BCM_COMMENT_RE_LEV : 0, //원문인 경우 comment_re_seq는 0,
									//comment_re_ref는 댓글의 원문 글번호
				BCM_COMMENT_RE_SEQ : 0
			},
			type: 'post',
			success : function(rdata) {
				alert(rdata ==1);
		...
		$('.comment_inobx_count').text(''); //입력한 글 카운트 초기화
...
$(".comment_list").on('click','.comment_tool_button', function() {
	...
	if($('.comment_list+.CommentWriter').css('display')=='block'){
		selector.toggle();
			...
			//더보기를 여러개 선택하더라고 최종 선택한 더보기 한개만 보이도록 합니다.
...
//더보기를 선택한 후 수정이나 삭제를 클릭할 경우 수정과 삭제 영역이 사라지게 합니다.
	...
	$('.CommentBox').on('click','.update', function(){
		...
		$.ajax({
			url:'CommentUpdate.bk',
			data:{BCM_NO:$(this).attr('data-id'), BCM_CONTENT: content},
			success:function(rdata){
				if(rdata==1){
					getList(option);
				}//if
