...
public class NTListActionEnr implements Action {
	  ...
	  public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		  ...
		  NTDAO boarddao = new NTDAO();
			...
			if(searchType=="" && search=="") {
				...
				boardlist = boarddao.getBoardSearchList(page,limit,searchType,search);
			...
			/*
			 * 총 페이지 수
	=(DB에 저장 된 총 리스트의 수 + 한 페이지에서 보여주는 리스트의 수 - 1)/한 페이지에서 보여주는 리스트의 수 
	 		 *
	 		 * 예를 들어 한 페이지에서 보여주는 리스트의 수가 10개인 경우
	 		 * 예1) DB에 저장된 총 리스트의 수가 0이면 총 페이지 수는 0 페이지
	 		 * 예2) DB에 저장된 총 리스트의 수가 (1~10)이면 총 페이지 수는 1 페이지
	 		 * 예3) DB에 저장된 총 리스트의 수가 (11~20)이면 총 페이지 수는 2 페이지
	 		 * 예4) DB에 저장된 총 리스트의 수가 (21~30)이면 총 페이지 수는 3 페이지
			 */
			int maxpage = (listcount + limit - 1) /limit;
			//System.out.println("총 페이지 수=" + maxpage);
			
			/*
			 * startpage : 현재 페이지 그룹에서 맨 처음에 표시 될 페이지 수를 의미합니다.
			 ...
			 *  11~20  페이지의 내용을 나타낼 때는 페이지 그룹은 [11][12][13]...[20]까지 표시됩니다.
			 */
			 ...
			 * 시작 페이지(startpage-21)와 마지막 페이지 (endpage=30)이지만
			 * 최대 페이지(maxpage)가 25fkaus [21]~[25]까지만 표시되도록 합니다. 
			 */
	     		if(endpage > maxpage)
	     		   endpage = maxpage;
			...
			if(state == null) {
				...
				request.setAttribute("listcount", listcount); //총 게시글의 수
